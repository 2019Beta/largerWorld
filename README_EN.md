...
- ChunkStatus dependencies expand across cell boundaries. The real vanilla generation entry point applies interactive-first admission, bounded speculative backpressure, and global write-set leases; tune it with `largerworld.chunkTasks.maxActive` and `largerworld.chunkTasks.maxQueuedPrefetch`.
- Repeated pending chunk saves coalesce to the latest snapshot, NBT conversion stays lazy until consumption, failed Region writes retry, and unload/Cell-close barriers wait for queued durability. Tune retries with `largerworld.chunkIo.maxWriteAttempts` and `largerworld.chunkIo.retryDelayMillis`.
